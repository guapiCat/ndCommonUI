 ...
 * @param {Number} value
 */
export function reversedSize(value) {
    if (value < 1024) {
        return value + 'KB'
    } else {
        let size
        size = (value / 1024).toFixed(2) + 'MB'
        return size
    }
}
...
export function getWaittime(curSize, openFileSizeTimeScope) {
    ...
    var fileSize0 = parseInt(scope[0].fileSize) * 1024; //KB
    if (curSize <= fileSize0) {
        waitTime = scope[0].waitTime;
        return waitTime;
    }
    var fileSize_end = parseInt(scope[scope.length - 1].fileSize) * 1024;
    ...
    for (let i = 0; i < scope.length - 1; i++) {
        ...
        var fs = parseInt(obj.fileSize) * 1024;
        var fs1 = parseInt(obj1.fileSize) * 1024;
